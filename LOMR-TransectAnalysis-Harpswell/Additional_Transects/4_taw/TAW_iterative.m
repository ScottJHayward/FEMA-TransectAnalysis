...
file=xlsread('../ADCIRC_returns/CM-144-1XYZSTA_RETURNS.csv');
sta_full=file(:,4);
ele_full=file(:,3);
...
diary logfiles/CM-158-2-DIARY.txt  % open a diary file to record calculations
diary on         % begin recording

% FEMA appeal for The Town of Harpswell, Cumberland county, Maine
% TRANSECT ID: CM-158-2
% calculation by SJH, Ransom Consulting, Inc. 26-Feb-2020
% 100-year wave runup using TAW methodology
% including berm and weighted average with foreshore if necessary
%
...
% CONFIG
%--------------------------------------------------------------------------
fname='inpfiles/CM-158-2sta_ele_include.csv';   % file with station, elevation, include
                                      % third colunm is 0 for excluded points
imgname='logfiles/CM-158-2-runup';                            
SWEL=8.8547;  % 100-yr still water level including wave setup. 
H0=1.1951;    % significant wave height at toe of structure 
Tp=1.6123;    % peak period, 1/fma, 
T0=Tp/1.1;   
...
file=xlsread('../ADCIRC_returns/CM-158-2XYZSTA_RETURNS.csv');
sta_full=file(:,4);
ele_full=file(:,3);
