...
diary logfiles/YK-76F-DIARY.txt  % open a diary file to record calculations
diary on         % begin recording

% FEMA appeal for The Town of Wells, York county, Maine
% TRANSECT ID: YK-76F
% calculation by SJH, Ransom Consulting, Inc. 05-Mar-2020
...
% CONFIG
%--------------------------------------------------------------------------
fname='inpfiles/YK-76Fsta_ele_include.csv';   % file with station, elevation, include
                                      % third colunm is 0 for excluded points
imgname='logfiles/YK-76F-runup';                            
SWEL=8.8908;  % 100-yr still water level including wave setup. 
H0=4.5874;    % significant wave height at toe of structure 
Tp=13.8735;    % peak period, 1/fma, 
T0=Tp/1.1;   
...
setupAtToe=1.055; 
maxSetup=1.4462;   % only used in case of berm/shallow foreshore weighted average 

plotTitle='Iterative TAW for YK-76F'
...
file=xlsread('../ADCIRC_returns/YK-76FXYZSTA_RETURNS.csv');
sta_full=file(:,4);
ele_full=file(:,3);
...
diary logfiles/YK-76-DIARY.txt  % open a diary file to record calculations
diary on         % begin recording
...
% CONFIG
%--------------------------------------------------------------------------
fname='inpfiles/YK-76sta_ele_include.csv';   % file with station, elevation, include
                                      % third colunm is 0 for excluded points
imgname='logfiles/YK-76-runup';                            
SWEL=8.8908;  % 100-yr still water level including wave setup. 
H0=4.5874;    % significant wave height at toe of structure 
Tp=13.8735;    % peak period, 1/fma, 
T0=Tp/1.1;   
...
setupAtToe=1.055; 
maxSetup=1.4316;   % only used in case of berm/shallow foreshore weighted average 

plotTitle='Iterative TAW for YK-76'
...
file=xlsread('../ADCIRC_returns/YK-76XYZSTA_RETURNS.csv');
sta_full=file(:,4);
ele_full=file(:,3);
