...
for i=1:length(tnums)
    ...
    %only for each station specified, select toe/top
        ...
        axis([first_cross-500 first_cross+500 -20 40])
        ...
        axis([wha_start(i)-100 topx(i)+500 -20 50])
        plot([wha_start(i) wha_start(i)],[-100 100],'c')
        scatter([toex(i) topx(i)],[toez(i) topz(i)],'md')
        legend('Transect Profile','ADCIRC Profile','TWL','HS','TP','1-D Wave start','TAW toe/top','location','northeast')
%         set(gcf,'position',[100 100 900 600],'paperorientation','landscape')
        ...
        set(gca,'tickdir','out');
        set(gcf,'position',[0 0 600.0000  740.8000])
%         Map(gca,'ocm',[],2)
        Map(gca);
